...
namespace ImGuiNETWidget
{
    ...
    public class MemoryEditor
    {
        ...
        public unsafe void Draw(string title, byte[] mem_data, int mem_size, int base_display_addr = 0)
        {
            ...
            ImGui.BeginChild("##scrolling", new Vector2(0, 0), false, 0);
            ...
            for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible items
            {
                ...
                for (int n = 0; n < Rows && addr < mem_size; n++, addr++)
                {
                    ...
                    if (DataEditingAddr == addr)
                    {
                        ...
                        TextEditCallback callback = (TextEditCallbackData* data) =>
                        {
                        ...
                        var flags = InputTextFlags.CharsHexadecimal | InputTextFlags.EnterReturnsTrue | InputTextFlags.AutoSelectAll |
                            InputTextFlags.NoHorizontalScroll | InputTextFlags.AlwaysInsertMode | InputTextFlags.CallbackAlways;
